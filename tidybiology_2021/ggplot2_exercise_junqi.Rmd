...
Take a quick look at contents of this dataset before attempting the question below
```{r}
glimpse(scRNAseq)
head(scRNAseq)
```
...
What is the distribution transcript lengths for the genes in this dataset? Hint - the relevant variable is `transcript_length`
```{r}
...
How would you give each boxplot its own colour?
```{r}
ggplot(three_organ_df, aes(x = organ, y = transcript_length)) +
  geom_boxplot(aes(colour = organ)) +
  geom_jitter(colour = 'red') +
  labs(x = "Organ names",
       y = "Transcript Length",
       title = "Distribution of Transcript Length for Genes in each Organ")
...
Instead of having all the points be blue, change their colours according to which organ type they correspond to
```{r}
ggplot(three_organ_df, aes(x = cell_1, y = cell_2)) +
...
ggplot(three_organ_df, aes(x = cell_1, y = cell_2)) +
  geom_point(aes(colour = organ),
             size = 1.5,
             alpha = 0.3) +
  facet_wrap(vars(organ), labeller = label_both) # ~organ also works in place of vars(organ)
```
...
Give the previous plot appropriate labels for the x- and y-axes, and a title. Also make the x- and y-axis values bold, and remove the colour legend
```{r}
