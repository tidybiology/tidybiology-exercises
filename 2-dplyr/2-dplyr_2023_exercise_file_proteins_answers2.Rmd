---
title: "dplyr Exercise: proteins_answers"
...
arrange(filter(select(proteins,uniprot_id,length),length>500),length)
...
proteins %>% 
  select(uniprot_id,sequence,length,mass)
```
...
proteins %>% 
  filter(gene_name=="TP53",length<400)
```
...
proteins %>% 
  filter(protein_name!="Collagen alpha-1",length>=1000)
```
...
proteins %>% 
  mutate(mass_length=mass/length)
```
## `summarise()` 
...
proteins %>% 
  summarise(mean_lenght=mean(length),mean_mass=mean(mass))
```
## `group_by()`
...
size_by_initial_amino_acid<-proteins %>%
  mutate(first_letter = stringr::str_extract(sequence, pattern = "[[:alpha:]]")) %>%
  group_by(first_letter) %>% 
  summarise(mean(length))
```

Finally, save `size_by_initial_amino_acid` as a new file (e.g. csv) using `write.csv` 
```{r csv_save, eval = FALSE, echo = TRUE}
write.csv(size_by_initial_amino_acid,"InitialAA_Averagelenght.csv")
```
